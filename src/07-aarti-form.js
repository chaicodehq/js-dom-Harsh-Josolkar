...
export function setupAartiForm(formElement, onSuccess, onError) {
  ...
  const handleSubmit = (event) => {
    ...
    const vType = validateAartiType(aartiType);

    const errors = [];
    if (!vName.valid) errors.push(vName.error);
    if (!vDate.valid) errors.push(vDate.error);
    if (!vType.valid) errors.push(vType.error);
